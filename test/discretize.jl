...
# One observed
@named simple_system_1 = ODESystem(D.(x) .~ p .* x, tspan = (0.0, 2.0),
    observed = [
        y₁ ~ first(x)])
...
#  Two observed
@named simple_system_2 = ODESystem(D.(x) .~ p .* x, tspan = (0.0, 2.0),
    observed = [
        y₁ ~ first(x),
        y₂ ~ sqrt(x)])
...
@named simple_system_3 = ODESystem(D.(x) .~ p .* x .+ c, tspan = (0.0, 2.0),
    observed = [
        y₁ ~ first(x),
        y₂ ~ sqrt(x)])
...
@testset "Univariate" begin
    ...
    @test timegrid.timegrids[1] == [(0.0, 1.0), (1.0, 2.0)]
    @test timegrid.timespans == [(0.0, 1.0), (1.0, 2.0)]
    @test DynamicOED.get_tspan(timegrid, 1) == (0.0, 1.0)
...
@testset "Two Observed and Control" begin
    ...
    @test timegrid.timegrids[1] ==
          [(t_i, t_j) for (t_i, t_j) in zip(0.0:0.1:1.9, 0.1:0.1:2.0)]
