...
groundtruth_equations = ModelingToolkit.scalarize(vcat(
    D.(x) .~ p .* x,  # Sytem
    0. .~ x .- D.(G) .+ p .* G, # Sensitivity
    D.(F) .~  G.^2 .* w, # FIM DEQ
    Q .~ G # Unweighted Information
))
...
@test all((x) -> isequal(x...), zip(vcat(observed(oed), groundtruth_equations[end]), observed(reduced_oed)))
...
@test isa(ODAEProblem(reduced_oed, initials, (0., 10.)), ODEProblem)